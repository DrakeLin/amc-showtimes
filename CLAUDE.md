...
**Letterboxd film identity**: there is no public Letterboxd API and its search endpoint 403s our scraper, so `get_lb_data` guesses slugs (`/film/<slug>/`, then `-<year>`, then `-<year>-1` for collisions). Slugs collide across films sharing a title, and the wrong one often *does* have a rating while the film we want doesn't — so every candidate page is checked against AMC's `directors` and `releaseDateUtc` year (`amc._film_matches`) and rejected on mismatch. Without this, "The Odyssey" resolved to the 1997 miniseries and "Obsession" to an unrelated 2026 short. Year comparison allows AMC's year to run up to 2 ahead of Letterboxd's, since Letterboxd dates a film by its festival premiere.
